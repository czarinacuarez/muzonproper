  ...
  return (
    <>
      <IconButton onClick={handleOpen} variant="text">
        <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform">
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className="size-6 h-5 w-5 text-blue-gray-500"
          >
            <path
              strokeLinecap="round"
      ...
      <Dialog open={open} handler={handleOpen} className="p-2">
        <DialogHeader className="flex justify-center p-0">
          <Typography variant="h3" color="green">
            User Identity
          </Typography>
        </DialogHeader>
        <DialogBody className="relative flex w-auto max-w-3xl justify-center p-0">
          {userId ? (
            <>
              {!qrCodeGenerated ? (
                <Button
